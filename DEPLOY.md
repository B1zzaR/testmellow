...
Резервные копии хранятся в именованном томе Docker `pgbackups`, управляемом контейнером `pg_backup` (ежедневно, хранятся последние 7).

Контейнер защищён от классов «тихий сбой»: дампит во временный файл, проверяет его размер ≥ 1 КиБ перед публикацией и не ротирует существующие бэкапы при ошибке `pg_dump` или `gzip`. Сообщения вида `FATAL: pg_dump failed` или `FATAL: backup too small` ищите в логах контейнера.

```bash
# Список существующих резервных копий
docker exec vpn_pg_backup ls -lh /backups/

# Логи контейнера — алёрты «FATAL: …» означают, что бэкап не сделан
docker logs vpn_pg_backup --tail 50
...
> ⚠️ **Off-host копии — обязательны для production.** Том `pgbackups` живёт на том же хосте, что и сама БД. При компрометации/потере хоста бэкапы уходят вместе с продакшеном. Минимально настройте cron-задачу, которая раз в сутки копирует свежий файл из тома в S3/Backblaze/Hetzner Storage Box (и желательно — шифрует через `gpg --symmetric` перед отправкой).
